...
import com.lowryengineering.database.socket.Util;
  ...
  public static Request deserializeRequest(InputStream from, byte[] intBuff) {
      try {
        ...
        if (bodyLen > 1024 * 1024 * 1024) {
          throw new com.lowryengineering.database.query.DatabaseException("Invalid inner body length: " + bodyLen);
        }
        if (len > 1024 * 1024 * 1024) {
          throw new com.lowryengineering.database.query.DatabaseException("Invalid command length: " + len);
        }
        byte[] b = new byte[len];
        from.read(b);
        ...
        throw new com.lowryengineering.database.query.DatabaseException(e);
      }
   }
