...
public class BenchEmbeddedOtherDb {
...
//    Connection embeddedConn = DriverManager.getConnection("jdbc:h2:~/test", "sa", "");
    ...
    for (int i = 0; i < threads.length; i++) {
      ...
      threads[i] = new Thread(() -> {
        try (Connection conn = DriverManager.getConnection("jdbc:hsqldb:testdb", "sa", "")) {
    ...
    final Thread[] rangeThreads = new Thread[threadCount];
    for (int i = 0; i < rangeThreads.length; i++) {
      rangeThreads[i] = new Thread(() -> {
        try (Connection conn = DriverManager.getConnection("jdbc:hsqldb:testdb", "sa", "")) {
          try (PreparedStatement stmt = conn.prepareStatement("select * from persons where id >= 0")) {
    ...
    final Thread[] identityThreads = new Thread[threadCount];
