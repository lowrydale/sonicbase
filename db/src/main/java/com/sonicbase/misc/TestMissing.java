...
//        }
      ...
      if (offset++ % 100_000 == 0) {
        int currCount = (offset - (group * 100_000_000));
  ...
  private static void load(Connection conn, String[] args) throws SQLException {
    int group = Integer.valueOf(args[2]);
    long begin = System.currentTimeMillis();
    for (int offset = group * 100_000_000; offset < (group + 1) * 100_000_000;) {
      ...
      for (int i = 0; i < 200; i++) {
        ...
        stmt.setString(6, "m");
        if (offset++ % 100_000 == 0) {
          int count = (offset - (group * 100_000_000));
