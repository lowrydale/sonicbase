...
//ORIGINAL LINE: volatile AtomicJava<Node<K,V>> next;
			 ...
			 */
			bool appendMarker(Node<K1, V1> *f)
			{
				return casNext(f, f);
			}
			 ...
			 * @param f successor
			 */
			void helpDelete(Node<K1, V1> *b, Node<K1, V1> *f)
			{
				/*
...
//ORIGINAL LINE: Index<K,V>[] idxs = (Index<K,V>[])new Index[level+1];
		 ...
		 */
		void addIndex(Index<K, V> *idx, HeadIndex<K, V> *h, int indexLevel)
		{
	...
	public:
		 ...
		 */
		ConcurrentSkipListMap() : comparatorField(NULL)
		{
			initialize();
		}
