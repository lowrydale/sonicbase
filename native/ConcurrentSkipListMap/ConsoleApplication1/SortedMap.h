...
namespace skiplist { template<typename T>class Comparator; }
namespace skiplist { template<typename E>class Set; }
namespace skiplist { template<typename E>class Collection; }
...
namespace skiplist
{
	 ...
	 * SortedSet}.)
	 *
	 * <p>All keys inserted into a sorted map must implement the <tt>skiplist.Comparable</tt>
	 ...
	 * the sorted map is to correctly implement the <tt>skiplist.Map</tt> interface.  (See
	 * the <tt>skiplist.Comparable</tt> interface or <tt>skiplist.Comparator</tt> interface for a
	 * precise definition of <i>consistent with equals</i>.)  This is so because
	 * the <tt>skiplist.Map</tt> interface is defined in terms of the <tt>equals</tt>
	 ...
	 * contract of the <tt>skiplist.Map</tt> interface.
	 *
	 ...
	 * single argument of type <tt>skiplist.Comparator</tt>, which creates an empty
	 * sorted map sorted according to the specified comparator.  3) A
	 * constructor with a single argument of type <tt>skiplist.Map</tt>, which
	 ...
	 * with a single argument of type <tt>skiplist.SortedMap</tt>,
	 ...
	 *   skiplist.SortedMap&lt;String, V&gt; sub = m.subMap(low, high+"\0");</pre>
	 *
	 ...
	 *   skiplist.SortedMap&lt;String, V&gt; sub = m.subMap(low+"\0", high);</pre>
	 *
	...
	class SortedMap : public Map<K, V>
	{
		 ...
		 */
	public:
		virtual Comparator<K> *comparator() = 0;
		 ...
		 * the beginIterator's own <tt>remove</tt> operation), the results of
		 * the iteration are undefined.  The set supports element removal,
		 * which removes the corresponding mapping from the map, via the
		 * <tt>skiplist.Iterator.remove</tt>, <tt>skiplist.Set.remove</tt>,
		 ...
		 * (except through the beginIterator's own <tt>remove</tt> operation),
		 ...
		 * reflected in the set, and vice-versa.  If the map is modified
		 * while an iteration over the set is in progress (except through
		 * the beginIterator's own <tt>remove</tt> operation, or through the
		 * <tt>setValue</tt> operation on a map entry returned by the
		 * beginIterator) the results of the iteration are undefined.  The set
		 * supports element removal, which removes the corresponding
		 * mapping from the map, via the <tt>skiplist.Iterator.remove</tt>,
		 * <tt>skiplist.Set.remove</tt>, <tt>removeAll</tt>, <tt>retainAll</tt> and
		 ...
		 *         sorted in ascending key order
		 */
		virtual Set<typename Map<K,V>::template Entry<K,V>*> *entrySet() = 0;
	};
