...
def purge_db_server(home, dns, port, shard):
    command = "ssh -n -f -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i " + home + '/amazonkeys/dale-aws2.pem  ubuntu@' + dns + ' \"sh -c \'rm -rf /data/database/*\'\"'
...
for i in range(0, len(shards)):
    host = shards[i]["host"]
    port = shards[i]["port"]
    purge_db_server(home, host, port + 10, i)
